    ...
    <Layout navigate={navigate}>
      ...
      <div style={{ maxWidth:720, margin:'0 auto', padding:'48px 24px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(200px, 1fr))', gap:12, marginBottom:40 }}>
          {[
            { name:'Hidden Halal', desc:'Mainstream spots you didn\'t know were halal' },
            { name:'Delicious Desi', desc:'Pakistani, Indian & South Asian favorites' },
            { name:'Mezze Musts', desc:'Mediterranean, Arab, Turkish & Afghan' },
            { name:'Soy Selects', desc:'Halal Asian — Chinese, Korean, Vietnamese' },
          ].map(item => (
            <div key={item.name} style={{ background:COLORS.cardWhite, border:`1px solid ${COLORS.border}`, borderRadius:12, padding:'16px' }}>
              <div style={{ fontFamily:"'DM Sans', sans-serif", fontSize:14, fontWeight:700, color:COLORS.textDark, marginBottom:4 }}>{item.name}</div>
              <div style={{ fontFamily:"'DM Sans', sans-serif", fontSize:12, color:COLORS.textLight }}>{item.desc}</div>
            </div>
          ))}
        </div>
        <div style={{ background:COLORS.greenLight, border:`1px solid rgba(15,77,42,0.15)`, borderRadius:16, padding:'24px' }}>
          <h3 style={{ fontFamily:"'Playfair Display', serif", fontSize:18, fontWeight:700, color:COLORS.textDark, marginBottom:8 }}>Get in Touch</h3>
          <p style={{ fontFamily:"'DM Sans', sans-serif", fontSize:14, color:COLORS.textMid, lineHeight:1.6, marginBottom:12 }}>For restaurant features, partnerships, or press inquiries:</p>
          <a href="mailto:hello@halalrated.com" style={{ fontFamily:"'DM Sans', sans-serif", fontSize:14, fontWeight:700, color:COLORS.green, textDecoration:'none' }}>hello@halalrated.com</a>
        </div>
      </div>
    </Layout>
  );
    ...
    <Layout navigate={navigate}>
      ...
      <div style={{ maxWidth:800, margin:'0 auto', padding:'56px 24px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(220px, 1fr))', gap:16, marginBottom:48 }}>
          {[
            { title:'Permanent Page', desc:'Your own dedicated page on halalrated.com — searchable and indexed by Google' },
            { title:'Instagram Reel', desc:'A professional faceless aesthetic Reel posted to @halalrated' },
            { title:'SEO Exposure', desc:'"Is [your restaurant] halal?" searches will lead directly to your page' },
            { title:'NOVA Reach', desc:"Targeted exposure to Northern Virginia's halal food community" },
          ].map(item => (
        ...
        <div style={{ textAlign:'center', background:COLORS.cardWhite, border:`1px solid ${COLORS.border}`, borderRadius:20, padding:'40px' }}>
          ...
          <p style={{ fontFamily:"'DM Sans', sans-serif", fontSize:15, color:COLORS.textMid, marginBottom:24, lineHeight:1.6 }}>Reach out and we'll be in touch within 24 hours.</p>
          <a href="mailto:hello@halalrated.com?subject=Restaurant Feature Request" style={{ display:'inline-block', background:COLORS.green, color:'white', borderRadius:12, padding:'14px 32px', textDecoration:'none', fontFamily:"'DM Sans', sans-serif", fontSize:15, fontWeight:600, marginBottom:12 }}>Email hello@halalrated.com →</a>
          <br />
          <a href="https://instagram.com/halalrated" target="_blank" rel="noreferrer" style={{ fontFamily:"'DM Sans', sans-serif", fontSize:13, color:COLORS.textMid, textDecoration:'none', marginRight:16 }}>DM on Instagram @halalrated</a>
          <span style={{ color:COLORS.textLight, fontSize:13 }}>·</span>
          <a href="https://tiktok.com/@halalrated" target="_blank" rel="noreferrer" style={{ fontFamily:"'DM Sans', sans-serif", fontSize:13, color:COLORS.textMid, textDecoration:'none', marginLeft:16 }}>TikTok @halalrated</a>
        </div>
      </div>
    </Layout>
  );
